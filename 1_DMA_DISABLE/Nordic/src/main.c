...
// SPI receive handler that will be triggered on GPIO interrupt
void spi_receive_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) {
    if (total_bytes_received >= MAX_DATA_SIZE) return;
    ...
    int err = spi_transceive(spi_dev, &spi_cfg, NULL, &(struct spi_buf_set){ .buffers = &(struct spi_buf){ .buf = recvbuf, .len = TOTAL_BYTES }, .count = 1 });
    ...
    if (err) {
        ...
        // Check the header to obtain the sequence number from the 7 Less Significant Bits
        ...
        // Check if we have received all the data (MAX_DATA_SIZE bytes)
...
void main(void) {
    ...
    // Configure the CS pin as output and ensuring it is inactive
    gpio_pin_configure(gpio_dev, SPI_CS_PIN, GPIO_OUTPUT);
    gpio_pin_set(gpio_dev, SPI_CS_PIN, 1);

    // Set up GPIO interrupt for DataReady pin
    gpio_init_callback(&data_ready_cb, spi_receive_handler, BIT(GPIO_DATAREADY));
    gpio_add_callback(gpio_dev, &data_ready_cb);
    gpio_pin_interrupt_configure(gpio_dev, GPIO_DATAREADY, GPIO_INT_EDGE_RISING);
