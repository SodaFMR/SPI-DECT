...
struct spi_config spi_cfg = {
    .frequency = 7500000,  // Puedes aumentar esta frecuencia según el límite de tu hardware
...
static bool transfer_complete = false; // Flag para detener recepción
...
void main(void)
{
    ...
    while (total_bytes_received < MAX_DATA_SIZE && !transfer_complete) {
        ...
        if (err) {
            ...
            // Verificar si el paquete recibido tiene menos datos de lo esperado
            if (recvbuf[bytes_to_copy] == 0x00) { 
                // Encontramos el final de la transmisión
            ...
            if (transfer_complete || total_bytes_received >= MAX_DATA_SIZE) {
                printk("All data received: %d bytes\n", total_bytes_received);
                break;
            }
        }

        // Reducir el retardo entre las lecturas SPI para aumentar la velocidad de recepción
        k_msleep(100); // Retardo reducido para aumentar la velocidad
    }
}
